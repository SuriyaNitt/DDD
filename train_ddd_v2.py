...
from keras.layers.core import Dense, Dropout, Activation, Flatten, Merge, Reshape, Lambda
...
def corr_model(frameHeight1, frameWidth1, frameHeight2, frameWidth2):
    ...
    modelForward.add(LSTM(32, return_sequences=True))

    modelBackward = Sequential()
    modelBackward.add(merged)
    modelBackward.add(LSTM(32, go_backwards=True, return_sequences=True))
    ...
    #model.add(Activation('softmax'))
    model.add(Lambda(lambda x: K.mean(x, axis=1), output_shape=lambda shape: (shape[0],) + shape[2:]))
...
def train(model, crossTrainTarget, crossTrainId, epoch):
    ...
    print('totalLoss:{}'.format(totalLoss))
